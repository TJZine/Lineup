...
3) **Plan (Codex)** → Keep the plan in Codex via `update_plan`. For multi-step work, reflect locked decisions, risks, verification commands, and rollback notes in the plan itself (or in `docs/plans/*` when tracked durable task memory is needed). Use [`docs/agentic/plan-authoring-standard.md`](./docs/agentic/plan-authoring-standard.md) for serious tracked plans, use `docs/runs/` for local-only major-task execution artifacts, and move older completed tracked plans to `docs/archive/plans/*` when they no longer need to stay active.
