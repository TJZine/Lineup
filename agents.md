...
- **Default workflow = risk-tiered orchestration**: Choose the lightest workflow that matches task risk. Tier 1: small bounded low-risk work uses a single agent plus review. Tier 2: a normal cleanup unit uses planner → implementer → reviewer. Tier 3: hotspot, cross-boundary, multi-session, or otherwise high-risk work uses the full controller loop. `docs/AGENTIC_DEV_WORKFLOW.md` is the repo workflow source of truth, and `docs/agentic/skill-strategy.md` is the skill-topology source of truth.
- **Codex multi-agent roles = tracked and conservative**: When using Codex multi-agent support, prefer the repo-defined role config in `.codex/config.toml` + `.codex/agents/*.toml`; plan before spawning; keep `explorer`/`reviewer`/`docs_researcher`/`monitor` read-only; use `explorer_fallback` or `monitor_fallback` only when spark usage is constrained; keep worker write scopes disjoint; wait sparingly.
...
- **UI skill order**: For UI creation or redesign, choose exactly one global UI skill by intent: use `interface-design` for product interfaces (dashboards/admin/settings/tools/data-heavy UI) and `frontend-design` for marketing/landing pages and other brand-forward surfaces; then load repo-local `ui-composition-patterns` for Lineup-specific TV composition, focus, motion, and cleanup rules.
...
3) **Choose orchestration tier** → Tier 1: single-agent plus review for small bounded low-risk work. Tier 2: planner → implementer → reviewer for a normal cleanup unit. Tier 3: controller loop for hotspot, cross-boundary, multi-session, or otherwise high-risk work.
...
5) **Verify (local, risk-based)** → run `npm run verify` for UI/navigation/Orchestrator/Plex work (or at least `npm run typecheck` + `npm test` for logic-only TS changes) before concluding.
