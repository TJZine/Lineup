...
| `src/modules/player/VideoPlayer.ts` | 1049 | Accepted current production hotspot baseline after PQR-5-S1 centralized player-owned async rejection construction in the player runtime owner; no routine line growth is allowed. | Revisit/decomposition trigger: any net line growth, ownership expansion, or future player-runtime extraction that can move structured failure construction out of the hotspot without changing public playback behavior. |
...
| `src/modules/plex/stream/resolver/PlexStreamResolver.ts` | 574 | Accepted current production hotspot baseline after PQR-5-S2 switched stream token query construction to the canonical Plex token key owner; no routine line growth is allowed. | Revisit/decomposition trigger: any net line growth, ownership expansion, or future stream URL/token owner extraction touching this file. |
