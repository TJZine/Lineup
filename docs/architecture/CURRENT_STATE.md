...
- `SelectedServerPersistenceAdapter` owns strict opaque evidence for selected or cleared candidate persistence and exact selected/unselected rollback proof. Evidence rejects port, active-profile, token/user identity, device-key, other-user selection, and partial-pair drift while allowing validation-owned refreshes of non-identity token metadata
...
- `src/core/channel-setup/build/ChannelSetupBuildCommitter.ts` owns durable channel replacement as the build commit boundary; committed summaries carry explicit commit authority, and aborts observed during the post-commit guide refresh are returned as staged interruptions without rolling back or misreporting saved channels as canceled
...
- `src/modules/ui/channel-setup/ChannelSetupSessionRuntime.ts` owns string-only UI runtime error summaries for load, preview/review, build, blocked, and bookkeeping outcomes; durable channel replacement remains authoritative across stale UI sessions, while post-commit guide aborts are represented as committed-with-guide-interrupted outcomes; typed planning/build failure details stay in core contracts/logs rather than `ChannelSetupScreen`
