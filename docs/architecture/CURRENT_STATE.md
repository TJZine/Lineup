...
- `ServerSelectionCoordinator` serializes select and clear commands on one tail. `selectServer()` retains the exact discovery receipt plus opaque persistence evidence across every continuation, suspends/drains ordinary tuning before scope work, and is the sole production constructor of `{ kind: 'selected', persistedSelection, epgRefresh }`; clear completes the coherent unselected runtime restoration before the command settles
