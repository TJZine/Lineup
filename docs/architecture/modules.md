...
- `src/core/app-shell/AppContainerFactory.ts` is the app-shell DOM owner that creates the bounded `runtime-chrome-host` under `#app` and keeps the approved runtime chrome members grouped there in fixed structural order
