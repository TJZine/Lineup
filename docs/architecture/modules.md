...
- `src/modules/ui/epg/EPGDebugRuntime.ts` owns only the bounded `lineup_debug_epg_log` cache/helper fan-out and is not precedent for new UI-layer storage owners
...
- `src/modules/debug/DebugOverridesStore.ts` owns the `lineup_debug_epg` flag; `src/modules/ui/epg/EPGDebugRuntime.ts` (`appendEpgDebugLog`) owns the bounded `lineup_debug_epg_log` cache and helper fan-out
