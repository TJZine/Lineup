...
- Keep the repo-defined role set conservative: read-only evidence/review/docs/monitor roles plus one bounded `planner` role for planning artifacts and one bounded `worker` role for implementation, with explicit fallback roles instead of assumed automatic failover.
