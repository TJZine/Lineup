...
- honor any `IMPLEMENTER_ROLE_ELIGIBILITY` in the approved `CURRENT_EXECUTION_PACKET`; use `worker` by default, `cleanup_worker` for Tier 3 cleanup-loop implementation passes, and `worker_terra` only for approved bounded exact cheap-to-verify units
