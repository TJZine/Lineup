...
9. when a weaker/cheaper implementer or an unusually fragile current unit needs extra detail, include an optional `CURRENT_EXECUTION_PACKET` block before `NEXT_SESSION_HANDOFF` with:
   - `UNIT`
   - `IMPLEMENTER_ROLE_ELIGIBILITY: worker_luna | worker | cleanup_worker`
   - `WHY`
   - `FILES_IN_SCOPE`
   - `FILES_OUT_OF_SCOPE`
   - `CONSTRAINTS`
...
Use `worker_luna` eligibility only for approved, bounded, exact, cheap-to-verify execution units. Keep `cleanup_worker` as the Tier 3 cleanup-loop default. Any packet allowing `worker_luna` must stop/escalate on ambiguity, plan contradiction, scope expansion, unexpected cross-boundary coupling, or verification failure needing diagnosis.
