...
- honor any current execution packet implementer eligibility, escalation, and stop/replan rules; use `worker` by default and `worker_luna` only for approved bounded exact cheap-to-verify units
