...
Default to the tracked Sol role for normal work. Use the cheaper Terra worker
...
- Score `0-1`
  ...
  - use `worker_terra` (`gpt-5.6-terra medium`) only when the approved `CURRENT_EXECUTION_PACKET` explicitly declares `IMPLEMENTER_ROLE_ELIGIBILITY: worker_terra` and the unit is exact, bounded, and cheap to verify
...
- Score `2-3`
  ...
  - implementer `gpt-5.6-terra medium` through `worker_terra` only for approved, bounded, exact, cheap-to-verify units with explicit stop/escalation rules
...
- Use `gpt-5.6-terra medium` through `worker_terra` for approved, bounded, exact, cheap-to-verify execution units.
- Use `gpt-5.6-terra medium` for documentation research and `gpt-5.6-terra high` for the explorer fallback.
...
- `worker_terra` eligibility requires `IMPLEMENTER_ROLE_ELIGIBILITY: worker_terra` or an eligibility set containing `worker_terra`, exact files, exact constraints, explicit verification, and stop/replan triggers; stop on ambiguity, plan contradiction, scope expansion, unexpected cross-boundary coupling, or verification failure needing diagnosis.
...
- `cleanup_worker` and cleanup/refactor implementers remain `gpt-5.6-sol medium` by default; route only explicitly exact, bounded, cheap-to-verify cleanup units to `worker_terra`.
- Preserve the current reasoning effort for the first GPT-5.6 run, then compare the same effort with one level lower on representative tasks. Reserve `max` for measured quality-first cases where `xhigh` is insufficient; do not make `max` or host-specific `ultra` a tracked default.
- Use `gpt-5.5` at the same effort as the reliability fallback for Sol/Terra roles when GPT-5.6 is unavailable. Use `gpt-5.4-mini` only for low-risk cost-sensitive work that would otherwise use Luna or a lightweight Terra role.
...
- Using `worker_terra` for ordinary or ambiguous implementation only to save cost
