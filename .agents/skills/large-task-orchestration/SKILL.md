...
description: Explicit controller workflow for very large Lineup features, refactors, migrations, or remediation programs that require multiple bounded subagents; do not use for routine work or a single delegated unit.
---
...
- use `worker_sol_low` through `bounded-worker-execution` when an exact unit still
  needs repository comprehension;
- use `worker_luna` through `bounded-worker-execution` only for frozen,
  low-ambiguity, repeatable, cheap-to-verify execution;
...
For Lineup, load only the boundary skills implicated by each unit. Changes to
architecture, persistence, Plex integration, UI composition, navigation,
`src/App.ts`, or `src/Orchestrator.ts` still require their runbook proof;
delegation never lowers the verification tier.
