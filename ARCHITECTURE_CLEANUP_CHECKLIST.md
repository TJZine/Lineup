...
- [ ] `P1-W5` finish runtime-root and facade narrowing for `AppOrchestrator` consumer seams
...
- [ ] `P1-W6` finish initialization and composition-bag narrowing
...
- [ ] `P1-EXIT` run the priority-exit review before moving to `P2`
  ...
  - Source-proof matrix (current code audit, 2026-03-28):
    ...
    - `review::.::holistic::abstraction_fitness::oversized_runtime_dependency_bags` -> `split follow-up` -> owner `P1-W6`; proof: `InitializationCoordinator` and coordinator-factory contracts remain wide (`src/core/InitializationCoordinator.ts`, `src/core/orchestrator/OrchestratorCoordinatorFactory.ts`)
    - `review::.::holistic::abstraction_fitness::single_impl_interface_ceremony` -> `split follow-up` -> owner `P1-W6`; proof: stale overlay/interface references are retired, but detector still reports residual ceremony and must be retired by the same initialization/composition-bag narrowing seam
  ...
  - Follow-up ownership and revisit triggers:
    ...
    - `split follow-up` to `P1-W6`: `review::.::holistic::abstraction_fitness::oversized_runtime_dependency_bags`, `review::.::holistic::abstraction_fitness::single_impl_interface_ceremony`; reason: same initialization/composition-bag seam remains live; revisit trigger: rerun each exact issue-id command at `P1-W6` closeout and retire or re-defer with one final owner
  ...
  - Exit state: `P1-EXIT` remains open until `P1-W5` and `P1-W6` retire or explicitly disposition their inherited issue sets on current evidence.
