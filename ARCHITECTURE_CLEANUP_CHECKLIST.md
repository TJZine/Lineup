...
- [ ] `P6-W3` narrow the remaining app-shell runtime facade and coordinator assembly bags so composition roots stay thin
  ...
  - Progress note (2026-04-09): extracted `src/core/server-selection/ServerSwapCoordinator.ts` so `AppOrchestrator.selectServer()` delegates the server-swap startup/EPG refresh workflow, and moved the app-shell-facing selected-server result contract to `src/core/server-selection/ServerSelectionTypes.ts`; the broader runtime-facade and dependency-bag narrowing below still remains open before `P6-W3` can close.
