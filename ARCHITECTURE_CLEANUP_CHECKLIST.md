...
- [ ] `P1-EXIT` run the priority-exit review before moving to `P2`
  ...
  - Required verification: `desloppify status`; `desloppify show review --status open --no-budget --top 100`; `desloppify show security --status open --no-budget --top 50`; all nine exact `P1` issue-id checks; `npm run verify`
  - Inherited follow-up from `P1-W1` (destination owner details): `review::.::holistic::cross_module_architecture::orchestrator_runtime_hub`; source disposition `split follow-up`; required verification command: `desloppify show "review::.::holistic::cross_module_architecture::orchestrator_runtime_hub" --status open --no-budget`; exit requirement: separate stale constructor evidence from any still-live runtime-hub debt before final disposition.
  - Inherited follow-up from `P1-W1` (destination owner details): `review::.::holistic::abstraction_fitness::orchestrator_facade_sprawl`; source disposition `split follow-up`; required verification command: `desloppify show "review::.::holistic::abstraction_fitness::orchestrator_facade_sprawl" --status open --no-budget`; exit requirement: separate stale constructor evidence from any still-live broad-facade debt before final disposition.
  - Inherited follow-up from `P1-W2` (destination owner details): `review::.::holistic::abstraction_fitness::oversized_runtime_dependency_bags`; source disposition `split follow-up`; required verification command: `desloppify show "review::.::holistic::abstraction_fitness::oversized_runtime_dependency_bags" --status open --no-budget`; exit requirement: confirm residual wide bag seams and runtime-role boundaries with one final owner.
  - Inherited follow-up from `P1-W2` (destination owner details): `review::.::holistic::abstraction_fitness::single_impl_interface_ceremony`; source disposition `split follow-up`; required verification command: `desloppify show "review::.::holistic::abstraction_fitness::single_impl_interface_ceremony" --status open --no-budget`; exit requirement: separate stale evidence from live interface ceremony debt before final disposition.
  - Inherited follow-up from `P1-W2` (destination owner details): `review::.::holistic::design_coherence::app_orchestrator_remains_multi_hub`; source disposition `split follow-up`; required verification command: `desloppify show "review::.::holistic::design_coherence::app_orchestrator_remains_multi_hub" --status open --no-budget`; exit requirement: disposition remaining orchestrator multi-hub responsibilities with a single final owner.
  - Inherited follow-up from `P1-W3` (destination owner details): `review::.::holistic::error_consistency::app_startup_contract_swallowed_failure`; source disposition `split follow-up`; required verification command: `desloppify show "review::.::holistic::error_consistency::app_startup_contract_swallowed_failure" --status open --no-budget`; exit requirement: rerun the issue-id check and disposition stale-vs-live startup-contract evidence against the current App.start rethrow + wrapper-owned bootstrap fatal-presentation contract.
  - Inherited follow-up from `P1-W3` (destination owner details): `review::.::holistic::error_consistency::orchestrator_precondition_strategy_drift`; source disposition `split follow-up`; required verification command: `desloppify show "review::.::holistic::error_consistency::orchestrator_precondition_strategy_drift" --status open --no-budget`; exit requirement: rerun the issue-id check and explicitly adjudicate whether the preserved `switchToChannel*` safe no-op path remains an intentional boundary split or requires further narrowing before `P1-EXIT` closeout.
  - Inherited follow-up from `P1-W4` (destination owner details): `review::.::holistic::high_level_elegance::composition_root_role_drift`; source disposition `split follow-up`; required verification command: `desloppify show "review::.::holistic::high_level_elegance::composition_root_role_drift" --status open --no-budget`; exit requirement: rerun the issue-id check and disposition the remaining composition-root role breadth with one final owner.
  - Inherited follow-up from `P1-W4` (destination owner details): `review::.::holistic::error_consistency::recovery_warning_paths_drop_error_context`; source disposition `split follow-up`; required verification command: `desloppify show "review::.::holistic::error_consistency::recovery_warning_paths_drop_error_context" --status open --no-budget`; exit requirement: rerun the issue-id check and disposition stale-vs-live recovery-warning evidence after the controller catch-branch removal.
  - Inherited follow-up from `P1-W4` (destination owner details): `review::.::holistic::test_strategy::orchestrator_module_factory_wiring_gap`; source disposition `split follow-up`; required verification command: `desloppify show "review::.::holistic::test_strategy::orchestrator_module_factory_wiring_gap" --status open --no-budget`; exit requirement: rerun the issue-id check and disposition stale-vs-live factory-wiring coverage evidence after the dedicated test slice landed.
