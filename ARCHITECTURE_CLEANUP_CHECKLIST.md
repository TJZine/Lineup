...
- [ ] `P1-EXIT` run the priority-exit review before moving to `P2`
  ...
  - Source-proof matrix (current code audit, 2026-03-28):
    ...
    - `review::.::holistic::design_coherence::app_orchestrator_remains_multi_hub` -> `split follow-up` -> owner `P1-EXIT`; proof: schedule-day rollover and subtitle-track recovery moved out, but `AppOrchestrator` still concentrates broad runtime state, cross-module wiring, and screen-facing runtime capabilities in one hotspot (`src/Orchestrator.ts`)
  ...
  - Follow-up ownership and revisit triggers:
    - `split follow-up` under `P1-EXIT`: `review::.::holistic::cross_module_architecture::orchestrator_runtime_hub`, `review::.::holistic::abstraction_fitness::orchestrator_facade_sprawl`, `review::.::holistic::design_coherence::app_orchestrator_remains_multi_hub`, `review::.::holistic::high_level_elegance::composition_root_role_drift`; reason: one live runtime-root seam remains around the broad app-shell facade and `AppOrchestrator` runtime concentration; revisit trigger: rerun each exact issue-id command during the next `P1-EXIT` review and either retire the seam on current source or mint one bounded owned follow-up before closing `P1-EXIT`
    - `split follow-up` under `P1-EXIT`: `review::.::holistic::abstraction_fitness::oversized_runtime_dependency_bags`; reason: the live residual is now narrowed to dependency-bag breadth in `NavigationCoordinator`, `InitializationCoordinator`, and `OrchestratorCoordinatorFactory`; revisit trigger: rerun the exact issue-id command during the next `P1-EXIT` review and either retire those bags on current source or hand them to one bounded owned follow-up
  ...
  - Residual scope note: the remaining live residual seam under `P1-EXIT` is limited to the runtime-facade and dependency-bag items above; `review::.::holistic::error_consistency::orchestrator_precondition_strategy_drift` has been split to `P5-W1` and is no longer part of `P1` closeout scope.
